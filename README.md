...
太极（taiji）不是一个“工具”，也不是某个理论的实现。它是一个**验证与证明的实验装置**：用 Rust 把一组关于智能的第一性原理写成可以编译、可以运行、可以自我审计的结构，然后让它跑起来，观察这两件事是否成立。
...
参数只是载体;能力是这个循环的结构本身。太极（taiji）的存在，是为了验证“能力可以被**从参数中剥离出来、放到循环里**”——而不是只能靠扩大参数规模去逼近。
...
太极（taiji）的全部结构，从这四条出发。它们不是口号，是每一层代码背后的假设。
