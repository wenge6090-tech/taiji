...
- **V41 根目录净化**：`LiluoClient::new` / `new_sparse` **不再创建资产层目录**（V41 前根下会有空的 models/prompts/skills/verifications——ensure_dirs 副作用，无消费者）。根目录只保留 `model_stats.yaml` + 分区子目录；资产层目录仅由 `for_model` 的分区 ensure_dirs 创建。未分区写路径仍可用（`save_asset` 内部自建父目录）；`scan_assets` 对缺失目录安全跳过。测试手写资产文件需自建目录（`create_dir_all` 后再 `fs::write`）。
